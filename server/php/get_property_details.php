...
try {
    ...
    $stmt = $conn->prepare('SELECT id, property_type, owner_name, state, district, taluk, kmc_area, ownership_type, notes, created_at, encumbrance_certificate, tax_paid_receipt, occupancy_certificate, zoning_certificate, status FROM property_verification WHERE id = ?');
    if (!$stmt) {
